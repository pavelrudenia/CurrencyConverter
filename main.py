...
def exchange():
    ...
    e_rur.delete(0, END)
    try:
        e_usd.insert(0, round(float(e_blr.get()) / float(JSON_object[5]['Cur_OfficialRate']), 2))
        e_eur.insert(0, round(float(e_blr.get()) / float(JSON_object[6]['Cur_OfficialRate']), 2))
        e_rur.insert(0, round((float(e_blr.get()) / float(JSON_object[17]['Cur_OfficialRate'])*100), 2))
    except:
...
usd_buy = Label(header_frame, text=JSON_object[5]['Cur_OfficialRate'], font="Arial 10")
usd_buy.grid(row=1, column=1, sticky=EW)
usd_sale = Label(header_frame, text=JSON_object[5]['Cur_OfficialRate'], font="Arial 10")
...
eur_buy = Label(header_frame, text=JSON_object[6]['Cur_OfficialRate'], bg="#ccc", font="Arial 10")
eur_buy.grid(row=2, column=1, sticky=EW)
eur_sale = Label(header_frame, text=JSON_object[6]['Cur_OfficialRate'], bg="#ccc", font="Arial 10")
eur_sale.grid(row=2, column=2, sticky=EW)

# RUR course
rur_currency = Label(header_frame, text="RUR(100₽)", font="Arial 10")
rur_currency.grid(row=3, column=0, sticky=EW)
rur_buy = Label(header_frame, text=JSON_object[17]['Cur_OfficialRate'], font="Arial 10")
rur_buy.grid(row=3, column=1, sticky=EW)
rur_sale = Label(header_frame, text=JSON_object[17]['Cur_OfficialRate'], font="Arial 10")
...
# BLR
l_blr = Label(calc_frame, text="BLR:", bg="#fff", font="Arial 10 bold")
l_blr.grid(row=0, column=0, padx=10)
e_blr = ttk.Entry(calc_frame, justify=CENTER, font="Arial 10")
e_blr.grid(row=0, column=1, columnspan=2, pady=10, padx=10, sticky=EW)
e_blr.insert(0, START_AMOUNT)
...
e_usd.insert(0, round(START_AMOUNT / float(JSON_object[5]['Cur_OfficialRate']), 2))
...
e_eur.insert(0, round(START_AMOUNT / float(JSON_object[6]['Cur_OfficialRate']), 2))
...
e_rur.insert(0, round((START_AMOUNT / float(JSON_object[17]['Cur_OfficialRate'])*100), 2))
